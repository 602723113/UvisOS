package cc.zoyn.uvisos;

import cc.zoyn.uvisos.handler.ActionType;
...
public class UvisOS extends JcqAppAbstract implements ICQVer, IMsg, IRequest {

	private static UvisOS INSTANCE;
	...
	public static UvisOS getInstance() {
		return INSTANCE;
	}
	
	public static void logInfo(String content) {
		CQ.logInfo("[UvisOS]", content);
	}
	...
	public String appInfo() {
		String AppID = "cc.zoyn.uvisos.uvisos";
		/**
	...
	public int startup() {
		...
		dataFolder = new File(CQ.getAppDirectory());
		// 根目录
	...
	public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
		...
		if (msg.startsWith("/")) {
			...
			if (msg.startsWith("/关键词")) {
				Handler.sendKeyWordHelp(fromQQ, fromQQ, ActionType.SEND_PRIVATE_MSG);
				return MSG_IGNORE;
			}
		}
	...
	public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ, String fromAnonymous, String msg,
		...
		KeywordManager.getWords().forEach((key, word) -> {
			if (msg.contains(key)) {
				CQ.sendGroupMsg(fromGroup, word);
			}
		});
		if (msg.startsWith("/")) {
			if (msg.startsWith("/help")) {
			...
			if (msg.startsWith("/code")) {
				Handler.sendCodeHelp(fromGroup, fromQQ, ActionType.SEND_GROUP_MSG);
				return MSG_IGNORE;
			}
			if (msg.startsWith("/关键词")) {
				Handler.sendKeyWordHelp(fromGroup, fromQQ, ActionType.SEND_GROUP_MSG);
				return MSG_IGNORE;
			}
		}
